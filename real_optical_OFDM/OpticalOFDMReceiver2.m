...
methods(Access = protected)
  ...
  function setupImpl(obj)
    ...
    % initialize the broadband modulator object and add necessary pad bits
    % to the signal
    setUpModulation(obj);
  ...
  function [y, numFramesDetected,numPayLoadBit,received_symbols] = stepImpl(obj, x)
    ...
    while bufferStartIdx <= lastBufferStartIdx
        ...
        if frameDetected
            ...
            %stop saving bit when number of frame detected > numframes
            if obj.pNumFrameDetected > obj.numFrames
                disp('number of frame for measurements is enought');
                obj.pNumFrameDetected = obj.pNumFrameDetected - 1;
            else
                %no display if payload bit is wrong (182 = known value for measurements)
                if numPayLoadBit ~= 182 
                    disp('an error occure during the demodulation of the header');
                    y = [y, decMsgInBits(1:end)'];
                    received_symbols = [received_symbols,demodulatedData];
                else  
                    %save payloadBits
                    y = [y, decMsgInBits(1:end)'];
                    received_symbols = [received_symbols,demodulatedData];
                    recovered_msgInbits = reshape(decMsgInBits(1:numPayLoadBit),[7, floor((length(decMsgInBits(1:numPayLoadBit)'))/7)])';
  ...
  % initialize broad band modulation objects
...
methods (Access = private)  
  ...
  % hermitian symmetry fro long preamble

classdef (StrictDefaults) OpticalOFDMTransmitter2 <matlab.System
...
methods        
  function obj = OpticalOFDMTransmitter2(varargin)
  ...
  function set.MsgInBits(obj, msgInBits)
    obj.MsgInBits = msgInBits;
  end
  ...
  function frameLen = get.FrameLength(obj)
    switch obj.ModulationType
          case 'BPSK'
              frameLen = ceil(length(obj.MsgInBits)/(obj.NumDataCarriers*obj.modulationOrder)) * ...
                   (obj.FFTLength+obj.CyclicPrefixLength) + obj.NumOFDMSymInPreamble*obj.FFTLength;
          case 'QPSK'
              frameLen = ceil(length(obj.MsgInBits)/(obj.NumDataCarriers*obj.modulationOrder)) * ...
                   (obj.FFTLength+obj.CyclicPrefixLength) + obj.NumOFDMSymInPreamble*obj.FFTLength;
          case 'QAM'
              frameLen = ceil(length(obj.MsgInBits)/(obj.NumDataCarriers*log2(obj.modulationOrder))) * ...
                   (obj.FFTLength+obj.CyclicPrefixLength) + obj.NumOFDMSymInPreamble*obj.FFTLength;
...
methods(Access = protected)
  function setupImpl(obj)
    ...
    %encode data
    obj.pPayloadBits = obj.MsgInBits;
    ...
    obj.pDataOFDMMod = comm.OFDMModulator(...
     ...
     obj.headerMod = ModulateHeader('pPayloadBits',obj.header,...
  ...
  function [y,symPostOFDM,FrameHeader,symPostQPSK] = stepImpl(obj)  
    ...
    % receiver modulated header
    FrameHeader = obj.headerMod();
    
    
    % concatennate modulated data and header
    y = [obj.pPreamble;FrameHeader];
    y = [y;symPostOFDM];
